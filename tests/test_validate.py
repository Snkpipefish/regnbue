"""Fase etter-MVP: OOS-validering — logikk på syntetisk panel."""

from __future__ import annotations

from setups.outcomes import Panel, PanelRow
from setups.validate import validate


def _row(date, vec, direction, r, oos):
    return PanelRow(date=date, vector=vec, direction=direction, outcome_r=r,
                    hit=(r > 0), oos=oos)


def test_validate_detects_predictive_signal():
    # Train: LONG-stater nær {d:0.5} ga vinnere (+2R); OOS-staten ligner → bør predikere bull.
    train = [_row(f"2020-01-{i+1:02d}", {"d": 0.5}, "LONG", 2.0, False) for i in range(40)]
    oos = [_row(f"2024-01-{i+1:02d}", {"d": 0.5}, "LONG", 2.0, True) for i in range(20)]
    v = validate(Panel("X", train + oos), similarity=0.15, min_effective_n=30,
                 min_hit_rate_pct=55, min_expectancy_r=0.3)
    assert v.n_train == 40 and v.n_oos == 20
    assert v.n_predicted == 20            # alle OOS har nok naboer
    assert v.sign_agreement == 1.0        # alle predikert + og realisert +
    assert v.n_gate_pass == 20            # sterk evidens → gaten publiserer
    assert v.gate_exp == 2.0


def test_validate_no_neighbors_when_too_far():
    train = [_row(f"2020-01-{i+1:02d}", {"d": 0.0}, "LONG", 2.0, False) for i in range(40)]
    oos = [_row(f"2024-01-{i+1:02d}", {"d": 0.9}, "LONG", 2.0, True) for i in range(20)]
    v = validate(Panel("X", train + oos), similarity=0.15, min_effective_n=30,
                 min_hit_rate_pct=55, min_expectancy_r=0.3)
    assert v.n_predicted == 0 and v.n_gate_pass == 0
